...
public:
	...
	int getDistance(nav_msgs::Path map_path);
...
private:
    ros::NodeHandle n_;
    ros::Subscriber odom_sub, path_sub, goal_sub, resule_sub, amcl_sub;
    ros::Publisher pub_, marker_pub, three_point_pub, cancle_pub, reach_pub;
    ...
    int index, i, j, mappath_size, result;
	...
	move_base_msgs::MoveBaseActionResult result_state;
	
	float amcl_pose_x = 0.0;
    float amcl_pose_y = 0.0;
	
	actionlib_msgs::GoalID aaa;
	
    double L, Lfw, Lrv, Vcmd, lfw, lrv, steering, u, v, prospect, k_far;
	double ks_circle, vel_circle, look_ahead_circle, ks_line, vel_line, look_ahead_line;
	...
	void resultCB(const move_base_msgs::MoveBaseActionResult::ConstPtr& resultMsg);
...
L1Controller::L1Controller()
{
    ...
    pn.param("controller_freq", controller_freq, 50);
	...
	pn.param("ks_circle", ks_circle, 0.4);
	pn.param("vel_circle", vel_circle, 0.3);
	pn.param("look_ahead_circle", look_ahead_circle, 0.1);
	
	pn.param("ks_line", ks_line, 0.5);
	pn.param("vel_line", vel_line, 0.5);
	pn.param("look_ahead_line", look_ahead_line, 0.5);
	...
	//path_sub = n_.subscribe("/move_base/GlobalPlanner/plan", 10, &L1Controller::pathCB, this);
	...
	resule_sub = n_.subscribe("/move_base/result", 10, &L1Controller::resultCB, this);
	
    amcl_sub = n_.subscribe("/amcl_pose",10,&L1Controller::amclCallback, this);
	...
	cancle_pub = n_.advertise<actionlib_msgs::GoalID>("move_base/cancel", 20);
	
	//pub_ = n_.advertise<geometry_msgs::Twist>("/cmd_vel", 10);
    pub_ = n_.advertise<geometry_msgs::Twist>("/cmd_vel_plan", 10);
	reach_pub = n_.advertise<std_msgs::Bool>("/reach_topic", 10);
...
void L1Controller::amclCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& amclPose)//ConstPtr
 {
...
{
    map_path = *pathMsg;
	mappath_size = int(map_path.poses.size());
...
void L1Controller::resultCB(const move_base_msgs::MoveBaseActionResult::ConstPtr& resultMsg)
{
	result_state = *resultMsg;
	
	if (result_state.status.status == 2)
	{
		result = 1;
	}
	else
...
double L1Controller::getCar2GoalDist()
{
    ...
    double car2goal_x = odom_goal_pos.x - amcl_pose_x;
    double car2goal_y = odom_goal_pos.y - amcl_pose_y;
    ROS_INFO("goal_pos = (%.2f,%.2f)", odom_goal_pos.x,odom_goal_pos.y);
	ROS_INFO("amcl_pose = (%.2f,%.2f)", amcl_pose_x,amcl_pose_y);
...
int L1Controller::getDistance(nav_msgs::Path map_path)
{
	//ROS_INFO("map_path.poses.size() = %d", int(map_path.poses.size()));
	//mappath_size = int(map_path.poses.size());
	ROS_INFO("map_path.poses.size() = %d", int(mappath_size));
	if (mappath_size<= 6)
		return 1;
	else
		return 0;
...
void L1Controller::goalReachingCB(const ros::TimerEvent&)
{
    if (goal_received)
    {
		...
		ROS_INFO("car2goal_dist: %f",car2goal_dist);
        if (car2goal_dist < 0.2)
		//if (getDistance(map_path))
        {
            goal_reached = true;
            goal_received = false;
...
{
    ROS_INFO("goal_reached = %d,goal_received = %d", goal_reached,goal_received);
		...
		if (map_path.poses.size() > 0)
		{
		   ...
		   /* if (abs(path_near_kappa) > 0.3 || abs(path_far_kappa) > 0.3)
			...
			if (abs(alpha) > 0.2)
			{
				...
				k_stanley = ks_circle;
				cmd_vel.linear.x = vel_circle;
				look_ahead = look_ahead_circle;
			...
			else
			{
				...
				k_stanley = ks_line;
				cmd_vel.linear.x = vel_line;//0.8
				look_ahead = look_ahead_line;
			}
