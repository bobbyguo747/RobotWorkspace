...
void msgCallback(const std_msgs::Int32MultiArray::ConstPtr& msg)
{
    ...
    for(int k=0;k<4;k++)
    {
        ...
        // ROS_INFO("sum_all=%d",sum_all);        
        if(msg->data.at(sum_all+k+2)==temp[0])
        {
            sum=msg->data.at(0+sum_all+k);
            sum_all+=sum;
            continue;
        }            
        else
        {
            ...
            order[0] = temp[0];
            ROS_INFO("Step No1: point %d",order[0]);
            for(int i=0;i<sum;i++)
            {
                order[i+1]=msg->data.at(sum_all+k+1+i);      
                ROS_INFO("Step No%d: point %d",i+1,order[i+1]);          
            }
            sum_all+=sum;
            break;
        }
...
void odomCallback(const nav_msgs::Odometry::ConstPtr& odom)//ConstPtr
 {
    ...
    if(flag==0)//还未发送途径点并收到最佳路径
    {
        ...
        ROS_INFO("next point:%d",next_point[temp[0]-1][2]);
        std_msgs::Int32MultiArray msg;
        msg.data.push_back(next_point[temp[0]-1][2]);
        for(int i=1;i<num;i++)
        {
