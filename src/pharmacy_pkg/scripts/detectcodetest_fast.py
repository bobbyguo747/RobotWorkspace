...
class FastQRTester(object):
    def __init__(self):
        ...
        rospy.logwarn('投票参数: 最近%d帧中同一结果达到%d次即输出' %
                      (self.vote_len, self.vote_need))
    ...
    def add_vote_and_maybe_output(self, task_data, all_data, source):
        ...
        self.votes.append((key, task_data, source, time.time()))
        ...
        rospy.logwarn_throttle(
            0.5,
            '候选=%s all_data=%s source=%s vote=%d/%d' %
            (str(key), safe_repr(all_data), source, best_count, self.vote_need)
        )

        if best_key == key and best_count >= self.vote_need:
