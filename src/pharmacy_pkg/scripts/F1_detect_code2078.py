...
# 有效任务二维码。空白和其他字符串都按无效二维码/干扰二维码处理。
VALID_CODES = ['A', 'B', 'C', 'AB', 'AC', 'BC', 'ABC']


def decode_area_all(img):
    """
    解码单个裁剪区域中的所有二维码。
    返回：
    raw_codes   : 该区域扫到的所有原始内容，包括干扰二维码。
    valid_codes : 该区域扫到的有效任务码，只保留 A/B/C/AB/AC/BC/ABC。
    """
    raw_codes = []
    valid_codes = []
    ...
    for qr in decode(img):
        try:
            ...
            data = str(qr.data).strip()

        if data not in raw_codes:
            raw_codes.append(data)

        if data in VALID_CODES and data not in valid_codes:
            valid_codes.append(data)

    return raw_codes, valid_codes


def code_to_msg_data(code, window_idx, error_count=0):
    """
    将任务码转换为原程序 /cam_return 使用的 Int32MultiArray。
    数组含义保持不变：
    [是否去C, 是否去A, 是否去B, 样本数, 化验窗口idx, 错误窗口]
    window_idx 为 0/1/2/3，对应裁判软件 1/2/3/4。
    """
    msg_data = [0, 0, 0, 0, 4, error_count]

    if code == 'A':
        msg_data = [0, 1, 0, 1, window_idx, error_count]
    elif code == 'B':
        msg_data = [0, 0, 1, 1, window_idx, error_count]
    elif code == 'C':
        msg_data = [1, 0, 0, 1, window_idx, error_count]
    elif code == 'AB':
        msg_data = [0, 1, 1, 2, window_idx, error_count]
    elif code == 'AC':
        msg_data = [1, 1, 0, 2, window_idx, error_count]
    elif code == 'BC':
        msg_data = [1, 0, 1, 2, window_idx, error_count]
    elif code == 'ABC':
        msg_data = [1, 1, 1, 3, window_idx, error_count]
...
# 新增：发布本帧识别到的全部有效二维码，导航程序用它做投票和去重。
pub_all_qr = rospy.Publisher('/cam_all_qr', String, queue_size=10)
...
# ================= 新版：收集四个窗口中的全部有效二维码 =================
            area_imgs = [enh_img1, enh_img2, enh_img3, enh_img4]

            all_data = []        # 本帧全部有效二维码任务
            raw_by_window = []   # 每个窗口扫到的原始内容，用于调试
            error_windows = []   # 出现干扰二维码的窗口，使用 1~4 编号

            for window_idx, area_img in enumerate(area_imgs):
                raw_codes, valid_codes = decode_area_all(area_img)
                raw_by_window.append(raw_codes)

                # 记录干扰二维码所在窗口。空白不算错误，非任务码才算错误。
                for raw_code in raw_codes:
                    if raw_code != "" and raw_code not in VALID_CODES:
                        error_windows.append(window_idx + 1)

                # 一个窗口理论上只有一个有效二维码；这里仍允许多个，增强鲁棒性。
                for code in valid_codes:
                    task = {
                        "window": window_idx,      # 0~3，保持原程序窗口编号
                        "pos": window_idx + 1,     # 1~4，方便看日志
                        "code": code,              # A/B/C/AB/AC/BC/ABC
                        "sample_count": len(code)  # 样本数，ABC=3，AC=2，A=1
                    }
                    all_data.append(task)

            if len(all_data) == 0:
                if raw_by_window != [[], [], [], []]:
                    rospy.logwarn("扫到二维码但均为无效内容 raw_by_window: %s", raw_by_window)
                continue

            # 兼容原 /cam_return 第 6 位错误窗口格式，只记录第一个错误窗口。
            Error_count = 0
            if len(error_windows) > 0:
                Error_count = error_windows[0]
                print("窗口 %d 出现错误信息" % Error_count)

            # 新增输出：本帧所有有效二维码。
            rospy.logwarn("识别到全部有效二维码 all_data: %s", all_data)
            payload = {"all_data": all_data}
            pub_all_qr.publish(json.dumps(payload))

            # 保留原 /cam_return 输出，兼容旧运动程序。
            # 选择原则：样本数多优先；样本数相同则窗口编号靠前优先。
            best_task = sorted(
                all_data,
                key=lambda x: (-x["sample_count"], x["window"])
            )[0]

            img_abc = best_task["code"]
            windows_count = best_task["window"]

            msg = Int32MultiArray()
            msg.data = code_to_msg_data(img_abc, windows_count, Error_count)

            rospy.logwarn(
                "本帧兼容输出: code=%s, pos=%d, msg=%s",
                img_abc,
                windows_count + 1,
                msg.data
            )
