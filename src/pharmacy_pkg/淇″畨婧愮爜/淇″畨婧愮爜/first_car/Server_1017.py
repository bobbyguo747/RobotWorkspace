...
rec_data = 0     # 用于通信传输时接收信息
enable_work = 1  # 双车行驶开关，1为1号车行驶，2为2号车行驶
go_to_ready = 0  # 双车起点等待标志，1为1号车去起点等待，2为2号车在起点等待
go_data = 0      # 用于通信传输时发送信息
# ram_result1 = [1, 1, 1, 3, 3]  # 用于socket传输二维码数据的列表
...
class MoveBaseSquare():
    def __init__(self):
        ...
        waypoints.append(Pose(Point(0.670, 2.568, 0), quaternions[1]))   # //A
        ...
        waypoints.append(Pose(Point(0.750,0.184,0), quaternions[9]))    # 识别板1
        waypoints.append(Pose(Point(-0.85, 0, 0), quaternions[10]))           # 起点后方的候车区

        # 发布TWist消息控制机器人
        # global ram_result1
        global go_data           # 用于通信传输时发送信息
        go_data = 0  # 用于通信传输时发送信息
        self.temp = 0            # 用于传输二维码数据给2号车
        self.count = 9           # 状态
        ...
        self.ram_result = [1, 1, 1, 3, 3, 0]
        # 订阅和初始化二维码数据2，用于2号车
        # self.cam_sub1 = rospy.Subscriber('/cam_return1', Int32MultiArray, self.detect_result1, queue_size=10)  # 订阅检测的药品和窗口数组
        # ram_result1 = [1, 1, 1, 3, 3]
        rospy.sleep(1)
        ...
        file.write("this line for test 测试 \n")

        while not rospy.is_shutdown():  # 如果ros系统没有关机的话

            # 2号车行驶时，1号车停留在等候区，等待2号车行驶到识别版2时发来的消息，之后行驶到起点等待，并恢复状态到初始值，同时接收消息等待出发
            if enable_work == 2 and go_to_ready == 1:  # 1号车前往起点等待
            ...
            if enable_work == 1:  # 1号车可以行驶
                rospy.loginfo("1号车开始行驶ovo")
                ...
                if self.count == 9:  # 从起点到识别区
                    ...
                    if self.move(goal) == True:
                        ...
                        # 到达识别版2后应发送消息，让2号车走到起点位置
                        go_data = 3  # 发送3，表示2号车该前往起点准备
                    ...
                    if self.move(goal) == True:
                        # 在候车区发送消息让2号车出发
                        rospy.loginfo("到达候车区!")
                        enable_work = 2    # 1号车跑完一圈后，关闭小车行驶开关，轮到2号车行驶
                        go_data = 1        # 发送1，表示2号车该出发了
                        # 在特定状态发送消息：发送1，表示2号车该出发了；发送3，表示2号车该前往起点准备
                        client_socket.send(str(go_data).encode('utf-8'))
...
def socket_server():
    ...
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 以AF_INET为协议族，SOCK_STREAM为类型创建socket
    server_socket.bind((host, port))  # 按照上文设置的地址和端口进行绑定
    server_socket.listen(5)  # 监听这个socket，等待客户端请求连接
    print('Server listening on {}:{}'.format(host, port))

    client_socket, addr = server_socket.accept()  # 接收客户端的连接请求
    ...
    while not rospy.is_shutdown():
        ...
        # 在收到特定消息后进行处理，enable_work = 1 1号车行驶，go_to_ready = 1 1号车前往起点
        if rec_data != '':
            print('Received data: {}'.format(rec_data))
            if int(rec_data) == 0:
                enable_work = 1
                rospy.loginfo("1号车收到出发指令")
            elif int(rec_data) == 2:
                go_to_ready = 1
                rospy.loginfo("1号车收到前往起点指令")
...
if __name__ == '__main__':
    # 创建ROS节点必须在主线程
    rospy.init_node('nav_pharmacy_server', anonymous=False)
