...
# 打开摄像头
cap = cv2.VideoCapture("http://192.168.166.248:8080/stream?topic=/camera/rgb/image_raw")
...
while not rospy.is_shutdown():
    ...
    if display:
        ...
        if (len(dingweikuang) == 48):
            ...
            if max_img == 3:
                # print("字母最大数量为 3")
                msg.data = [1, 1, 1, 3, windows_count, Error_count]  # 0.是否去c，1.是否去a，2.是否去b，3.样品数 4.self.windows_1234 5.错误样品窗口
                # print(msg.data)
                print("输出为%s" % msg.data)
