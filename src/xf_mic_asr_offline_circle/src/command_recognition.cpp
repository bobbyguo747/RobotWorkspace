...
***********************************/
	if(str1 == str2)
	{
		...
		OTHER = (char*) "/feedback_voice/shoudao.wav";//语音反馈
...
***********************************/
	else if(str1 == str3)
	{
        ...
        cmd_vel_flag_msg.data = 1;
        cmd_vel_flag_pub.publish(cmd_vel_flag_msg);
        OTHER = (char*) "/feedback_voice/shoudao.wav";
		WHOLE = join((head + audio_path),OTHER);
		system(WHOLE);
		cout<<"好的：小车后退"<<endl;
	}
...
int main(int argc, char** argv)
{
  ...
  /***创建自主导航目标点话题发布者***/
	...
	n.param<float>("/line_vel_x", line_vel_x, 0.3);
	n.param<float>("/ang_vel_z", ang_vel_z, 1.5);
	n.param("/if_akm_yes_or_no", if_akm, std::string("yes"));
  ...
  /***用户界面***/
	...
	if(if_six == "no")
	{
		cout<<"您可以语音控制啦！以降噪板设置的唤醒词为准[默认:“小易同学”]"<<endl;
		cout<<"小车前进———————————>向前"<<endl;
		cout<<"小车后退———————————>后退"<<endl;
		cout<<"小车左转———————————>左转"<<endl;
		cout<<"小车右转———————————>右转"<<endl;
