	...
	if (offline_recognise_switch == 1) //如果是离线识别模式
	{
		...
		if (whole_result!="")
		{
			...
			if (effective_ans.effective_confidence >= confidence) //如果大于置信度阈值则进行显示或者其他控制操作
			{
				...
				{
					rpl_flag = 1;
					OTHER = (char*) "/feedback_voice/wait_forever.wav";
		...
		else
		{
			...
			if(rpl_flag == 0)
			{
				OTHER = (char*) "/feedback_voice/request.wav";
				WHOLE = join((head + audio_path),OTHER);
				system(WHOLE);
			}
		}
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("source_path", source_path, std::string("~/robot_ws/src/xf_mic_asr_offline_circle"));
	...
	n.param("/command_recognition/audio_path", audio_path, std::string("~/robot_ws/src/xf_mic_asr_offline_circle/feedback_voice"));
	...
	while(ros::ok())
	{
		...
		{			
			finish_t = clock();
	        total_time_t = (double)(finish_t - start_t) / CLOCKS_PER_SEC;
			if(total_time_t > 16)
			{
				printf(">>>>>语音识别休眠5秒以上，可重新唤醒！\n");
				rpl_flag = 0;
				start_t = clock();
			}
		}
