...
class BaseControl:
    def __init__(self):
        ...
        if((base_type == 'NanoCar') & (self.sub_ackermann == True)):
            ...
            self.sub = rospy.Subscriber(self.cmd_vel_topic,Twist,self.cmdCB,queue_size=20) #接收导航算法发给他的速度指令
        self.pub = rospy.Publisher(self.odom_topic,Odometry,queue_size=10) #发布里程给导航算法
        self.battery_pub = rospy.Publisher(self.battery_topic,BatteryState,queue_size=3) #发布电池数据
        self.tf_broadcaster = tf.TransformBroadcaster() #发布tf树
        self.timer_odom = rospy.Timer(rospy.Duration(1.0/self.odom_freq),self.timerOdomCB) #用于计算里程的定时器，频率为50,（1秒钟计算50次），计算里程的回调函数为self.timerOdomCB
        self.timer_battery = rospy.Timer(rospy.Duration(1.0/self.battery_freq),self.timerBatteryCB) #串口指令每秒获取一次电池的电压数据。
        self.timer_communication = rospy.Timer(rospy.Duration(5.0/1000),self.timerCommunicationCB) #用于主要的数据交互，都在这个通讯程序里面
        #inorder to compatibility old version firmware,imu topic is NOT pud in default
        if(self.pub_imu):#不发布imu所以这块不需要
        ...
        self.getSN()##
        time.sleep(0.01)
        self.getInfo()##给底层板子发指令查询版本号，这些不需要，此处往上都没用。
    #CRC-8 Calculate
    ...
    def timerCommunicationCB(self,event): #此为通讯定时器的回调函数，非常重要。这个一直去读数据，度下位机返回来的数据。在数据接收器里面
        length = self.serial.in_waiting #等待串口返回来的数据长度。下面进行判断，如果发现一个完整的包的话就进行后续处理。
        if length:
    ...
    def timerOdomCB(self,event): #算里程发了一个查询速度数据的包，（做里程计的时候只需要xy以及角度这三个数）  发出数据包功能码为奇数，接收之后功能码为偶数（加一）
        ...
        while(self.serialIDLE_flag):
            time.sleep(0.01) #此处要等一下，等待接收完数据。
        self.serialIDLE_flag = 1
        try:
            while self.serial.out_waiting:
                pass
            self.serial.write(output) #此处将数据包发送出去
        except:
            rospy.logerr("Odom Command Send Faild")
        self.serialIDLE_flag = 0   
        #calculate odom data   计算Vxy的值以及角速度的值。
        ...
        self.pub.publish(msg) #最后把解析出来的里程计的数据打包成一个消息发布出去
    ...
    def timerBatteryCB(self,event):#py文件往底板发一个如下的数据包，用于获取电池电压相关数据。  包头 数据长度 地址号都为1 功能码后面跟着数据   两个校验位
    ...
    def timerIMUCB(self,event):  #此处查询imu原始属于都用不到了。框架里面算里程的时候只用了底层根据imu算出来的航向角就够了
