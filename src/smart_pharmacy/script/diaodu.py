...
def publisher():
    ...
    time0 = rospy.get_param('time1',5.0)    # 运行该程序后，停留多少时间（单位：秒），小车开始前进到识别区
    time1 = rospy.get_param('time1',15.0)     # 小车到达识别区，等待time1（单位：秒）后，走向配药区
    time2 = rospy.get_param('time1',2.0)     # 小车到达配药区，等待time2（单位：秒）后，走向答题区
    time3 = rospy.get_param('time1',10.0)     # 小车到达答题区，等待time3（单位：秒）后，走向取药区
    time4 = rospy.get_param('time1',2.0)     # 小车到达取药区，等待time4（单位：秒）后，走向起点
    time5 = rospy.get_param('time1',5.0)     # 小车到达起点，等待time5（单位：秒）后，下一轮出发
