...
camera_uri = rospy.get_param('camera_uri','http://192.168.12.1:8080/stream?topic=/camera/rgb/image_raw')
...
imgA = cv2.imread('A.jpg')
imgB = cv2.imread('B.jpg')
imgC = cv2.imread('C.jpg')
imgD = cv2.imread('NULL.jpg')
        ...
        if(len(dingweikuang)==48):
            ...
            for i in range(len(jieguo)):
                for j in range(len(jieguo[0])):
                    if jieguo[i][j][0][0] + jieguo[i][j][0][1] < min_dip:
                        min_dip = jieguo[i][j][0][0] + jieguo[i][j][0][1]
                        LU_index = [i,j]
                        # min_x_index_group.append(min_x_index)
                    if jieguo[i][j][0][0] + jieguo[i][j][0][1] > max_dip:
                        max_dip = jieguo[i][j][0][0] + jieguo[i][j][0][1]
                        RD_index = [i,j]
                    if jieguo[i][j][0][1] - jieguo[i][j][0][0] > max_dec_yx:
                        max_dec_yx = jieguo[i][j][0][1] - jieguo[i][j][0][0]
                        LD_index = [i,j]
                    if jieguo[i][j][0][0] - jieguo[i][j][0][1] > max_dec_xy:
                        max_dec_xy = jieguo[i][j][0][0] - jieguo[i][j][0][1]
                        RU_index = [i,j]
            # print(len(min_x_index_group))
            # print(min_x,max_x,min_y,max_y)
            # print(min_x_index,max_x_index,min_y_index,max_y_index)

            x0 = jieguo[LU_index[0]][LU_index[1]][0][0]
            y0 = jieguo[LU_index[0]][LU_index[1]][0][1]
            x1 = jieguo[RD_index[0]][RD_index[1]][0][0]
            y1 = jieguo[RD_index[0]][RD_index[1]][0][1]
            x2 = jieguo[RU_index[0]][RU_index[1]][0][0]
            y2 = jieguo[RU_index[0]][RU_index[1]][0][1]
            x3 = jieguo[LD_index[0]][LD_index[1]][0][0]
            y3 = jieguo[LD_index[0]][LD_index[1]][0][1]
            # print("x0 y0 :")
            # print(min_x_index)
            # print("x1 y1 :")
            # print(max_x_index)
            ...
            if len(contours0) > 0:
                ...
                # print('1: A matchShapes=', res1)
                # print('1: B matchShapes=', res2)
                # print('1: C matchShapes=', res3)
                # print('1: NULL matchShapes=', res4)
